...
<details><summary>Reading in the data</summary>
```{r}
library(dplyr)
...
PROC SQL;
  ...
  SELECT name, year, max(pressure) AS pressure, max(wind) AS wind
  FROM tmp1 
  GROUP BY year, name;
  ...
  SELECT year, name, mean(pressure) AS pressure, mean(wind) AS wind, category, status, mean(time) AS time
  FROM tmp1
  GROUP BY year, name;
QUIT;
