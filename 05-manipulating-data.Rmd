...
<details><summary>`arrange()` - sort rows in the table by one or more variables</summary>
```{r}
...
[The thing that makes the `tidyverse` philosophy so addictive is that it makes these common, everyday tasks both easy and concise (that is, few lines of code are required). ]{.my-opinion}
...
PROC SQL;
CREATE TABLE pvtmp AS 
SELECT * ,
CASE WHEN victim_s_race='Unknown Race' THEN 'Unknown race' ELSE victim_s_race AS race 
...
police_violence %>%
  ...
  summarize(age = mean(age, na.rm = T), name_length = mean(nchar(`Victim's name`)))
```
...
`group_by` is the verb that accomplishes the first task. `summarize` (in the next section) accomplishes the second task and implicitly accomplishes the third as well. 

Let's try it out using the `storms` dataset that is part of the `dplyr` package:
