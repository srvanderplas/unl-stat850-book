...
## Module Objectives  {- #module12-objectives}
...
![The faces of debugging (by Allison Horst)](https://github.com/allisonhorst/stats-illustrations/raw/main/other-stats-artwork/debugging.jpg){fig-alt="A cartoon of a fuzzy round monster face showing 10 different emotions experienced during the process of debugging code. The progression goes from (1) “I got this” - looking determined and optimistic; (2) “Huh. Really thought that was it.” - looking a bit baffled; (3) “...” - looking up at the ceiling in thought; (4) “Fine. Restarting.” - looking a bit annoyed; (5) “OH WTF.” Looking very frazzled and frustrated; (6) “Zombie meltdown.” - looking like a full meltdown; (7) (blank) - sleeping; (8) “A NEW HOPE!” - a happy looking monster with a lightbulb above; (9) “insert awesome theme song” - looking determined and typing away; (10) “I love coding” - arms raised in victory with a big smile, with confetti falling."}
...
![A debugging manifesto [@evansDebuggingManifestoHttps2022]](images/debugging/b0rk-debugging-manifesto.jpeg){fig-alt="a debugging manifesto.
...
![Title: user.fist_name [@nasserUserFistName2020]](images/debugging/nasser_junior-misspelled.jpeg){fig-alt="A programmer clutches a laptop and says 'I hate this job'... 'and I hate my life' (while pulling at its face). The programmer screams 'WHY IS THIS HAPPENING TO MEEEE'. In the final frame, a calm programmer resumes typing and says 'nevermind, misspelled a variable.'"
width="60%"}
...
::: column-margin
![Debugging strategy: Shorten your feedback loop [@evansDebuggingStrategyShorten2022]](images/debugging/b0rk-shorten_feedback_loop.jpeg){fig-alt="strategy: shorten your feedback loop. When I'm debugging, I: 1. come up with a question 2. run the buggy code to answer my question 3. repeat dozens of times. If step 2 is slow, everything takes FOREVER. Some ways to speed it up: * use a browser automation tool! * write a unit test! * write a tiny program!"}
:::
...
::: column-margin
![Debugging strategy: Write a unit test [@evansDebuggingStrategyWrite2022]](images/debugging/b0rk-unit-test.jpeg){fig-alt="Image description
...
Do not be surprised if, in the process of debugging, you encounter new bugs. This is a problem that's well-known enough that it has its [own xkcd comic](https://xkcd.com/1739/). At some point, getting up and going for a walk may help. Redesigning your code to be more modular and more organized is also a good idea. 
...
Within 24h, I had gotten replies from people working at RStudio, and one of them had [diagnosed the problem](https://github.com/rstudio/reticulate/issues/1133#issuecomment-1021783041). After I asked a few more questions, one of them submitted a pull request to my repository with a solution.
...
Use [this list of StackOverflow posts](files/Debugging_exercise.html) to try out your new debugging techniques. Can you figure out what's wrong? What information would you need from the poster in order to come up with a solution?
:::
...
To prevent the site from changing on me (as websites tend to do...), I've saved the HTML file [here](data/realclearpolitics_frag.html).
...
![Both Show Traceback and Rerun with Debug are useful tools](images/debugging/Rstudio_rerun_traceback.png){fig-width="50%"
...
To run code using `ipdb` when your code hits an error, add `
from ipdb import launch_ipdb_on_exception` to the top of your python code chunk. Then, at the bottom, put any lines that may trigger the error after these two lines:
...
![The first thing I see when I run lm after turning on debug (screenshot)](images/debugging/07_debug_init.png)

![The variables passed into the lm function are available as named and used in the function. In addition, we have some handy buttons in the console window that will let us 'drive' through the function](images/debugging/07_debug_console.png)

After pressing "next" a few times, you can see that I've stepped through the first few lines of the lm function.

![Stepping through the function. The arrow on the left side in the editor window shows which line of code we're currently at.](images/debugging/07_debug_console2.png)
...
![I can hit "Stop" or type "Q" to exit the debug environment.](images/debugging/07_debug_console_exit.png)
