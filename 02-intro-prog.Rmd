...
![](https://github.com/jennybc/lego-rstats/raw/master/lego-rstats_019.jpg?raw=TRUE)
Note that even though the vectors in the list are all the same size in this case, they don't have to be, because they're not organized in any sort of cohesive rectangle shape. 
...
If you have trouble distinguishing between `$`, `[`, and `[[`, you're not alone. The [R for Data Science book has an excellent illustration](https://r4ds.had.co.nz/vectors.html#lists-of-condiments)
...
There are also [lists in SAS IML which function similarly to lists in R](https://documentation.sas.com/?docsetId=imlug&docsetTarget=imlug_langref_sect040.htm&docsetVersion=15.1&locale=en). To create a named object in a list, precede the name with `#`. In SAS, the `$` operator can be used to get items from a list, using either 
...
The lego variant of a data frame looks like this: 

![](https://github.com/jennybc/lego-rstats/raw/master/lego-rstats_020.jpg?raw=TRUE)
...
The syntax for conditional statements using if/then logic is shown below using an example where Santa must determine which members of a household will receive a toy for Christmas and which members will receive coal. ^[Traditionally, naughty children got coal, while nice children got toys or candy.]
