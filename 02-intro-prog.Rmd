...
```{r out.width = "50%", fig.cap = "This is basically the class summarized. ", echo = F}
...
#### Type Conversions {#type-conversions-R}
...
#### Type Conversions {- #type-conversions-sas}
...
```{r sas-type-conversions-1, engine = "sas", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
data set1;
  x = 3;
  y = '3.1415';
  z = x * y;
...
```{r sas-type-conversions-2, engine = "sas", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
data set2;
  x = 3;
  y = '3.1415';
  z = x * y;
...
```{r clear-all-sas-output, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F, include = F}
```
...
<details><summary>Solutions</summary>
```{r tryitout-data-type-answers-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data set1; 
  string1 = 'Bob';
  string2 = 'Jane';
  x = string1 < string2;
...
```{r tryitout-data-type-answers-sas2, error = T, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data set2;
  string1 = 'Bob';
  y = string1 + 3;
  put y=;
...
```{r recursive-list-demo-sas, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
...
```{r mj-data-step, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data mathLSD;
...
### Indexing {#indexing-data-structures}
...
```{r matrix-example-sas, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
...
```{r vectorindexing-solution-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data tmp;
  do x = 1 to 999;
  output;
  end;
...
## Control structures {#control-structures-ch2}

### If statements {#if-statements-control}
...
In a data step:
```{r santa-sas, echo = T, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F, error = T}
data santa;
  input name $ status $;
  datalines;
  Edison nice
...
```{r santa-notworking, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F, error = T}
data santa;
  input name $ status $;
  datalines;
  Edison nice
...
```{r santa-working-1, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data santa;
  input name $ status $;
  datalines;
  Edison nice
...
```{r santa-working-2, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data santa;
  input name $ status $;
  datalines;
  Edison nice
...
```{r tryitout-sas-sample-solution, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
proc iml;
  call randseed(342502837);
  x = sample(1:50, 20)`; 
  create sampledata from x [colname = "x"];
...
#### Count controlled loops (FOR loops) {#for-loops}
...
```{r doloop1, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
...
```{r doloop2, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data A;
  do i = 1 to 10; 
    put i=;
  end; /* This ends the loop definition */
...
```{r loop-examples, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data A;
y = 0;
do i = 5 to 0 by -0.5;
    put i=; 
...
```{r loop-examples2, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
data A;
y = 0;
...
```{r sas-tryitout-iml-loop, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
/* SAS IML example loop */
proc iml;
  current = 1; 
  prev = 0;
...
```{r sas-tryitout-iml-loop-solutions, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
/* SAS IML example loop */
proc iml;
  current = 1;
  prev = 0;
...
#### Condition-controlled loops (WHILE, DO WHILE) {#while-loops}
...
<details><summary>In SAS</summary>
```{r basel-problem-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
proc iml;
  i = 1;
  basel = 0;
  prev = -1;
...
```{r sas-limit-tryitout-solution, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
proc iml;
  x = 3;
  dist = 4 - x;
  fx = 0;
...
#### Other Loops and Interative Structures {#other-loops}

There are many different ways to implement iteration in any language, including very low-level controls like `repeat` (in R). Higher level iteration may include a FOREACH loop, where a series of commands is applied to a list or vector (the `*apply` commands in R are examples of this). An additional method of iteration that requires functions is the recursion (where a function calls itself). In every case, these alternative loop structures can be translated to for or while loops, if you're thoughtful about it (though it may be more convenient for some problems to use e.g. recursion). At this point, I suspect you're probably overwhelmed enough, so we'll skip the other types of iteration for the moment.


## Overgrown Calculators {#calculations-in-r-sas}
...
```{r iml-math-operators-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
proc iml; 
  x = do(1, 10, 1);
  y = do(3, 30, 3);
...
```{r matrix-operations-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
proc iml;
...
```{r invert-matrix-example-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F }
proc iml;
  mat = {1 2 3, 6 4 5, 7 8 9};
...
![You can do it!](https://raw.githubusercontent.com/allisonhorst/stats-illustrations/master/rstats-artwork/r_first_then.png)

This is probably pretty overwhelming, but remember -- at this point, your job is to be able to remember where you saw the material and go back and look it up before applying it to a new problem. You can use a cheat-sheet, you can take notes, or you can just look through the book when doing your homework - but just like learning a spoken language, it's going to be kinda slow and challenging at first and you'll make lots of mistakes. I promise, you will get there :)

## References and Links {- .learn-more #refs-module-2}
