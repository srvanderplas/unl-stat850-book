...
[The EDA chapter in R for Data Science [\@r4ds] is very good at
explaining what the goals of EDA are, and what types of questions you
will typically need to answer in
EDA](https://r4ds.had.co.nz/exploratory-data-analysis.html). Much of the
...
We can also generate a mosaic plot (the equivalent of a 2-dimensional
...
```{r poke-graphs-base-cts, out.width = "24%", fig.show = "hold", collapse = T}
...
library(ggplot2)
p1 <-   ggplot(poke, aes(x = height_m)) + 
  geom_histogram(bins = 30)
p2 <- ggplot(poke, aes(x = height_m)) + 
  geom_histogram(bins = 30) + 
  scale_x_log10()
p3 <- ggplot(poke, aes(x = height_m)) + 
  geom_density()
p4 <- ggplot(poke, aes(x = height_m)) + 
  geom_density() + 
  scale_x_log10()

# This code arranges the 4 plots in a single row
library(gridExtra)
...
Note: This example uses the python `patchworklib` package to arrange
plots for the sake of the book layout. I do NOT expect you to be able to
use this package, and the code shown is actually only the code required
to make the plots, not to arrange them (for simplicity sake).
...
```{python poke-ggplot2-cts-py-execute, echo = F}
#| fig-show: hold
#| fig-keep: last
#| fig-width: 12
#| fig-height: 3
...
g1 = ggplot(poke, aes(x = 'height_m')) + geom_histogram(bins = 30)

g2 = (ggplot(poke, aes(x = 'height_m')) + 
  geom_histogram(bins = 30) + 
  scale_x_log10())

g3 = (ggplot(poke, aes(x = 'height_m')) + 
  geom_density())

g4 = (ggplot(poke, aes(x = 'height_m')) + 
  geom_density() + 
  scale_x_log10())
...
vis_patch = cow.patch(g1, g2, g3, g4) + cow.layout(design = np.array([0, 1, 2, 3]))
vis_patch.show(width=12, height = 3, dpi = 600)
```
...
The janitor package has functions for cleaning up messy data. One of its
best features is the `clean_names()` function, which creates names based
on a capitalization/separation scheme of your choosing.

![janitor and clean_names() by Allison
Horst](https://github.com/allisonhorst/stats-illustrations/raw/master/rstats-artwork/janitor_clean_names.png)
:::
