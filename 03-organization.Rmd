...
```
```{r load-libraries3, echo = F, message = F, warning = F, cache = F}
library(SASmarkdown)
sasexe <- "/usr/local/SASHome/SASFoundation/9.4/bin/sas_en"
sasopts <- "-ls 75"
```
```{r clear-all-sas-output3, include=F, engine='saslog', engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
...
## Organization: Module Objectives  {- #module-3-objectives}
...
SASmarkdown will not work if you don't have the following setup(s):

- Windows + SAS (not Community Edition) + R 
- Mac + Parallels (with SAS and RStudio both installed in parallels)
- Linux + SAS + R

If you're using Community Edition, you cannot use SASMarkdown. Instead, you'll have to submit SAS files separately from your Rmarkdown code, or use the server set up by HCC that has the correct configuration.
...
```{r iml-function-demo, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
proc IML;
...
```{r tryitout-circle-area-solution}
...
```{r tryitout-circle-area-solution2}
circle_area <- function(r) {
  if (!is.numeric(r)) {
...
```{r tryitout-circle-area-solution-sas, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
proc IML;
...
```{r scope-example-sas-1, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC IML;
a = 3;
...
```{r scope-example-sas-2, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F, error = T}
PROC IML;
a = 3;
...
```{r sas-scope-example-1, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F, error = T}
PROC IML;
start myfun(x); 
  y = 2 * x;
...
```{r argument-passing-sas, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC IML;
a = 3;
...
```{r sas-arg-by-reference, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC IML;
start myfun(x); 
...
```{r sas-arg-by-reference2, engine = "saslog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC IML;
...
```{r sas-prog-output-prediction, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F, error = T}
proc IML;
...
```{r sas-proc-options, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
...
```{r sas-proc-var, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC MEANS DATA=SASHELP.CARS;
  VAR msrp cylinders;
RUN;
```
...
```{r sas-proc-class, engine = "sashtmllog", engine.path = sasexe, engine.opts = sasopts, collectcode = F}
PROC MEANS DATA=SASHELP.CARS;
  CLASS type;
  VAR msrp cylinders;
RUN;
...
```{r sas-snowflake-tryitout, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F, fig.path = "image/", error = T}
PROC IML;
...
run;
```
```{r sas-reset-snowflake, engine = "sashtml", engine.path = sasexe, engine.opts = sasopts, collectcode = F, fig.path = "image/", error = T}
dm log "clear";
dm output "clear";
ods html close;
ods html;
...
<details><summary>Solution</summary>
```{r tryitout-tibble-pkg-solution, eval = -c(1, 3:4)}
