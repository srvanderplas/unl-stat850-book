...
You may have noticed that I've been including SAS chunks throughout this book, and even in your homework assignments. Here's how that's set up.
...
If you want to avoid too many issues with scoping (because scoping rules are complicated), the simplest way is to not reuse variable names inside of a function if you've already used those names outside the function.
...
Example: Argument passing in R and SAS    
In SAS:
<details>
...
Here's another example of SAS passing arguments by reference (and the unexpected effects that can have on a program's state): 
<details>
...
```
The pipe takes the left hand side and a function, and puts the left hand side as an argument to the function on the right hand side. It doesn't sound very impressive, but it allows you to do some very cool things: you can "chain" operations.
...
Normally, you'd write `f3(f2(f1(4)))`, which you have to read from the inside to the outside. With pipes, you can write the same operation as:
```{r}
4 %>% f1() %>% f2() %>% f3()
```
...
You don't need to know how to define your own infix operators, but you will want to become familiar with the pipe eventually. 
...
```{r, eval = F}
source("https://raw.githubusercontent.com/srvanderplas/unl-stat850/master/code/03_setup.R")
```
...
Both SAS and R have systems for extending the base system/language with additional functionality. In R, these extensions are called packages, and in SAS they are called modules.^[Note that the various components of SAS can be *extremely* confusing to separate. I found [this](https://www.lexjansen.com/mwsug/2017/RF/MWSUG-2017-RF01.pdf) guide to be somewhat helpful.]
...
In SAS, there are also packages of code that encapsulate scripts. However, unlike R (and many other languages), there is no centralized repository for SAS packages. Papers may include SAS packages to demonstrate new methods, and other packages may be found on GitHub or various SAS forums. 
