...
```{r tweets-more-ch5, include = F, echo = F, fig.cap = "Apologies for the noninclusive language, but the sentiment is real.", eval = F}
...
#### R {-}
```{r ggplot-power-pressure-r}
ggplot(max_power_storm, aes(x = time, y = pressure)) + geom_point()
```

#### Python {-}
```{python ggplot-power-pressure-py}
ggplot(max_power_storm, aes(x = "time", y = "pressure")) + geom_point()
```
...
storm_evolution["age"] = storm_evolution.groupby(["name", "year"]).apply(lambda x: x.time - x.time.min()).reindex()
