...
##### Python data setup
```{python}
legos_time = pd.read_csv("data/lego_sets.csv").groupby("year")[["theme_id"]].agg("count").reset_index()
...
ggplot(legos_time, aes(x = "year", y = "theme_id")) + geom_line()
...
```{python seaborn-line}
plt.cla()
sns.lineplot(data = legos_time, x = "year", y = "theme_id")
plt.show()
```
...
- 2D density heatmap
- 2D density w/ lines
- 2D density with polygons
- use of rayshader to get a 3D plot
