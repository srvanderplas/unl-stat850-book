...
::: note
Due to UTF-8 issues, this code chunk runs (with complaints) when I paste it into SAS, but does not run in bookdown. The complaints are because there are column names that are not valid SAS names (presumably, SAS doesn't allow UTF-8 characters as column names).
:::
...
```{sashtml proc-freq-demo-setup, include = F, collectcode = T}
ODS HTML style= HTMLBlue; 
```
```{sashtml proc-freq-demo, fig.path = "image/", collectcode = T}
libname classdat "sas/";
...
```{sashtml proc-means-demo-setup, include = F, collectcode = T}
ODS HTML style= HTMLBlue; 
```
```{sashtml proc-means-demo, fig.path = "image/", collectcode = T}
libname classdat "sas/";
...
```{sashtml proc-univariate-demo-setup, include = F, collectcode = T}
ODS HTML style= HTMLBlue; 
```
```{sashtml proc-univariate-demo, fig.path = "image/", collectcode = T}
libname classdat "sas/";
...
<details><summary>Solution</summary>
```{sashtml police-violence-tryitout-setup, include = F, collectcode = T}
ODS HTML style= HTMLBlue; 
```
```{sashtml police-violence-tryitout, fig.path = "image/", collectcode = T}
libname classdat "sas/";
