...
<details><summary>
Box plots aren't the only way to show distributions, though. If we want to, we can show distributions using histograms or density plots. A histogram involves binning the variable, counting the number of observations that fall within each specified range. Usually, these ranges have constant width (but not always). </summary>
...
You should always look at  a histogram under multiple bin widths, because that parameter can change the visual appearance of the distribution quite a bit. In base R, you can get histograms with the `hist()` function. 
...
A density plot is an empirical method of estimating the probability density function of a variable. Thus, instead of count, the y axis in a density plot will be labeled "density" and the curve will be scaled so that the distribution integrates to 1. Density plots can be quite useful for continuous variables, but are also dependent on parameter selection - instead of a bin width, you may need to adjust a kernel width. 
...
If you're careful to specify your mappings properly, you can plot a density curve on top of a histogram. Since both geoms compute statistics, you have to read the help files to see that they actually are computing multiple statistics.

::: tryitout
Read the help files and attempt to create your own histogram/density combination, before looking at the solution below
:::

<details><summary>Histogram/Density combo</summary>
...
You can use density plots and histograms in two dimensions to show the bivariate relationship between two variables, but at that point we have to represent the height of the density using another variable (as x and y are taken by the two variables you're computing a density for). 
...
```{r, fig.cap = "The color constancy illusion. The squares marked A and B are actually the same color", out.width = "45%"}
...
```{r, echo = F, fig.cap = "Which of the lines is the longest? Shortest? It is much easier to determine the relative length of the line when the ends are aligned. In fact, the line lengths are the same in both panels."}
segs <- bind_rows(
