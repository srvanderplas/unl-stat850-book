...
```{sashtml sas-rand-density-09, error = T}
...
<details><summary>In SAS</summary>
```{sashtml sas-linear-reg-09, error = T}
DATA tmp;
...
Under scenario 2, the expected value of the lognormal distribution is $\exp(1/2) = `r exp(.5)`$, the variance is $(\exp(1) - 1)(\exp(1)) = `r (exp(1) - 1)*exp(1)`$ and our theoretical interval should be $(`r exp(.5) - 1.96*sqrt((exp(1) - 1)*exp(1))/5`, `r exp(.5) + 1.96*sqrt((exp(1) - 1)*exp(1))/5`)$. Note that this interval contains 0, which is implausible for lognormally distributed data. 
...
```{sashtml sas-dog-simulation-09, error = T}
DATA dogtemp;
  call streaminit(20572983);
  DO i = 1 to 1000;
