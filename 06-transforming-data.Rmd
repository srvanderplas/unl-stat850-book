...
```{sashtml sas-college-read-data, fig.path = "image/"}
...
```{sashtml sas-college-find, fig.path = "image/"}
libname classdat "sas/";
...
```{sashtml sas-college-split-string-words, fig.path = "image/"}
libname classdat "sas/";
DATA collegename;
SET classdat.college;
...
```{sashtml sas-table3-scan, fig.path = "image/"}
data table3;
...
```{sashtml sas-table5-concatenate, fig.path = "image/"}
/* read in the data */
DATA table5;
...
```{sashtml sas-transpose-hiv-clean, error = T, warning = T, fig.path = "image/"}
libname classdat "sas/";
...
```{sashtml sqlite3-nycflights, echo = F, fig.path = "image/"}
...
```{sashtml flights-delay-age-sas-prep, collectcode=T, fig.path = "image/"}
libname nycair odbc complete = "dsn=nycflight; Database=data/nycflights13.sqlite";
...
```{sashtml flights-delay-age-sas-res, collectcode=T, fig.path = "image/"}
...
```{sashtml flights-delay-age-sas-plot1, collectcode=T, fig.path = "image/"}
...
```{sashtml flights-delay-age-sas-plot2, collectcode=T, fig.path = "image/"}
...
```{sashtml gasprices-sas-read-in-data, collectcode=T, error = T, fig.path = "image/"}
libname classdat "sas/";
...
```{sashtml gasprices-sas-showdata1, collectcode=T, echo = F, error = T, fig.path = "image/"}
libname classdat "sas/";

PROC PRINT data = classdat.gas_raw(obs=10);
RUN;
```

<details><summary>Split up year and month, format month properly</summary>

```{sashtml gasprices-sas-split-ym, collectcode=T, error = T, fig.path = "image/"}
libname classdat "sas/";
...
drop i ym; 
if date1 = "NA" then delete; /* get rid of 
run;

/* format month as a number */
data classdat.gas_split;
set classdat.gas_split;
format month $ mon.;
run;
```
</details>

```{sashtml gasprices-sas-showdata2, collectcode=T, error = T, fig.path = "image/"}
libname classdat "sas/";
PROC PRINT data = classdat.gas_split (obs=10);
RUN;
```
...
```{sashtml gasprices-sas-wide-to-long, collectcode=T, error = T, fig.path = "image/"}
PROC TRANSPOSE DATA=classdat.gas_split OUT =classdat.split_long1
...
data classdat.split_long1;
set classdat.split_long1;
IF _N_ = 1 THEN DO;
...
PROC SORT data = classdat.split_long1 out = classdat.split_long1;
BY year month week;
RUN;
```
</details>

```{sashtml gasprices-sas-showdata3, collectcode=T, error = T, fig.path = "image/"}
PROC PRINT data = classdat.split_long1 (obs=10);
RUN;
```

<details><summary>Get long data with just the dates</summary>
```{sashtml gasprices-sas-wide-to-long2, collectcode=T, error = T, fig.path = "image/"}

PROC TRANSPOSE DATA=classdat.split OUT =classdat.split_long2
...
data classdat.split_long2;
set classdat.split_long2;
IF _N_ = 1 THEN DO;
...
PROC SORT data = classdat.split_long2 out = classdat.split_long2;
BY year month week;
RUN;
...
```{sashtml gasprices-sas-showdata4, collectcode=T, error = T, fig.path = "image/"}
PROC PRINT data = classdat.split_long2 (obs=10);
RUN;
```

<details><summary>Merge the two long datasets together</summary>
```{sashtml gasprices-sas-wide-to-long-merge, collectcode=T, error = T, fig.path = "image/"}
PROC SQL;
CREATE TABLE classdat.gas_prices AS
SELECT COALESCE(p1.year, p2.year) AS year,
       COALESCE(p1.month, p2.month) AS month,
       COALESCE(p1.week, p2.week) AS week,
       date, price
FROM classdat.split_long1 as p1
RIGHT JOIN classdat.split_long2 as p2
ON p1.year = p2.year AND p1.month = p2.month AND p1.week = p2.week;
```
</details>


```{sashtml gasprices-sas-showdata5, collectcode=T, error = T, fig.path = "image/"}
PROC PRINT data = classdat.gas_prices (obs=10);
RUN;
```

<details><summary>Format dates</summary>
```{sashtml gasprices-sas-date-format, collectcode=T, error = T, fig.path = "image/"}

DATA classdat.split2;
   SET gas_prices;
   length var1-var2 3;
   array var(2);
   do i = 1 to dim(var);
...
PROC SORT DATA=classdat.gas_prices OUT = classdat.gas_prices;
BY date;
RUN;
```
</details>
...
```{sashtml gasprices-sas-showdata6, collectcode=T, error = T, fig.path = "image/"}
PROC PRINT data = classdat.gas_prices(obs=10);
RUN;
```

```{sashtml gas-prices-sas-plot, collectcode = T, error = T, fig.path = "image/"}

PROC SGPLOT DATA = classdat.gas_prices;
